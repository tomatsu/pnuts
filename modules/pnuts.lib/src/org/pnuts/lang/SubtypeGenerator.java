...
public class SubtypeGenerator extends Runtime {
	...
	{
		ClassFile cf = ClassGenerator.createClassFile(className, superClass, interfaces, null);
		constructor(cf, superClass);

		cf.addField("_package", "Lpnuts/lang/Package;", (short)(Constants.ACC_PRIVATE|Constants.ACC_STATIC));
		...
		if (signatures != null && typeMap != null){
			...
			if (!sigset.contains(s)){
			    generateGetter(cf, name, type, s.getMethodName());
			}
			s = Compiler.setterSignature(type, name);
			if (!sigset.contains(s)){
			    generateSetter(cf, name, type, s.getMethodName());
			}
		    }
	...
	static void generateGetter(ClassFile cf,
				   String name,
				   Class type,
				   String methodName)
	{
		...
		cf.openMethod(methodName, "()" + param, Constants.ACC_PUBLIC);
		cf.add(Opcode.ALOAD_0);
		cf.add(Opcode.GETFIELD, className, name, param);

		if (param.length() == 1){ // primitive
	...
	static void generateSetter(ClassFile cf,
				   String name,
				   Class type,
				   String methodName)
	{
	    String className = cf.getClassName();
	    String param = Signature.makeSignature(type);
	    cf.openMethod(methodName, "(" + param + ")V", Constants.ACC_PUBLIC);
	    cf.add(Opcode.ALOAD_0);
	    if (param.length() == 1){ // primitive
		char ch = param.charAt(0);
		switch (ch){
		...
		case 'Z':
		    ...
		    cf.add(Opcode.PUTFIELD, className, name, param);
		    break;
		case 'J':
