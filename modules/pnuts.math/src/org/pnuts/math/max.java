...
public class max extends PnutsFunction {
	...
	protected Object exec(Object[] args, final Context context){
		...
		if (nargs == 1){
			...
			if (type1 == 3){  // default comparator
				switch (type0){
				case 1: // Collection	
					return Collections.max((Collection)arg0);
				case 2: { // int[]
					int[] array = (int[])arg0;
					if (array.length < 1){
						return null;
			...
			switch (type0){
			case 1: // Collection	
				return Collections.max((Collection)arg0, comp);
			case 2: { // int[]
				int[] array = (int[])arg0;
				if (array.length < 1){
					return null;
	...
	public String toString(){
		return "fuction max(arg1, arg2) or (collection/array {, comparator | f(arg) })";
	}
}
