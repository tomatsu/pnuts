...
public class Main {
	...
	public static void main(String args[]) throws Throwable {
		...
		if (use_compiler) {
			...
			if (!accessible
					&& !"false".equals(System
							.getProperty("pnuts.compiler.useDynamicProxy"))) {
				defaultSettings.setProperty("pnuts.compiler.useDynamicProxy",
						"true");
			}
			if (optimize) {
		...
		ClassLoader pcl = Pnuts.createClassLoader(context);
		...
		if (interactive) {
			if (guiConsole) {
                                ...
                                ui.setTitle("Pnuts");
                                PnutsConsole console = ui.createConsole(context, inputlog);
                                console.start();
