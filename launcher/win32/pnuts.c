...
static void set_library_path(char* pnuts_home)
{
  ...
  if (lib){
    buf = malloc(9 + strlen(lib) + strlen(pnuts_home) + strlen(LIBRARY_PATH));
    if (buf == 0){
      perror("malloc");
      exit(-1);
    }
...
parseArgs(int* totalArgc, int* jvmArgc, int argc, char** argv)
{
    ...
    *jvmArgc = vmArgc;
    ...
    ext_arg = malloc(strlen(module_dir) + 24);
    if (ext_arg == 0){
	fprintf(stderr, "out of memory\n");
	exit(-1);
    }
    ...
    home_arg = malloc(16 + strlen(pnuts_home));
    if (home_arg == 0){
	fprintf(stderr, "out of memory\n");
	exit(-1);
    }
    ...
    if (modules){
        char* p = malloc(25 + strlen(modules));
	if (p == 0){
	    fprintf(stderr, "out of memory\n");
	    exit(-1);
	}
